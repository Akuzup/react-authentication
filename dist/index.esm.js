...
import { getAuth, connectAuthEmulator, GoogleAuthProvider, RecaptchaVerifier, signInWithEmailAndPassword, signInWithPopup, signInWithPhoneNumber, PhoneAuthProvider, signInWithCredential, createUserWithEmailAndPassword, updateProfile, signOut, sendPasswordResetEmail, EmailAuthProvider, reauthenticateWithCredential, updatePassword, sendEmailVerification, deleteUser, linkWithCredential, unlink, onAuthStateChanged } from 'firebase/auth';
import { getApps, initializeApp } from 'firebase/app';
import { getFirestore, connectFirestoreEmulator, doc, getDoc, serverTimestamp, setDoc, updateDoc, deleteDoc } from 'firebase/firestore';
...
class LoginUseCase {
    ...
    async execute(request) {
        try {
            ...
            if (!token) {
                throw AuthErrorFactory.createError(AuthErrorCode.TOKEN_REFRESH_FAILED, 'Failed to retrieve authentication token');
            }
            ...
            if (error instanceof Error) {
                throw AuthErrorFactory.createError(AuthErrorCode.INVALID_CREDENTIALS, 'Login failed', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
    async validateInput(request) {
        ...
        if (!emailValidation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.INVALID_EMAIL, emailValidation.errors.join(', '));
        }
        const passwordValidation = this.validationService.validatePassword(request.password);
        if (!passwordValidation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.WEAK_PASSWORD, passwordValidation.errors.join(', '));
        }
    }
}
...
class RegisterUseCase {
    ...
    async execute(request) {
        try {
            ...
            const token = await this.authRepository.getToken();
            if (!token) {
                throw AuthErrorFactory.createError(AuthErrorCode.TOKEN_REFRESH_FAILED, 'Failed to retrieve authentication token after registration');
            }
            ...
            if (error instanceof Error) {
                throw AuthErrorFactory.createError(AuthErrorCode.INTERNAL_ERROR, 'Registration failed', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
    async validateInput(request) {
        ...
        if (!passwordMatchValidation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.WEAK_PASSWORD, 'Passwords do not match');
        }
        // Validate display name if provided
        if (request.displayName) {
            const displayNameValidation = this.validationService.validateDisplayName(request.displayName);
            if (!displayNameValidation.isValid) {
                throw AuthErrorFactory.createError(AuthErrorCode.INVALID_EMAIL, // Using generic validation error
        ...
        if (request.phoneNumber) {
            ...
            if (!phoneValidation.isValid) {
                throw AuthErrorFactory.createError(AuthErrorCode.INVALID_PHONE_NUMBER, phoneValidation.errors.join(', '));
            }
        }
        ...
        if (!termsValidation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.PERMISSION_DENIED, 'You must accept the terms and conditions');
        }
    }
}
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('popup_blocked')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.POPUP_BLOCKED, 'Popup was blocked. Please allow popups and try again.');
                }
                if (error.message.includes('popup_closed_by_user')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.POPUP_CLOSED_BY_USER, 'Authentication was cancelled by user.');
                }
                if (error.message.includes('network')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.NETWORK_ERROR, 'Network error during Google authentication.');
                }
                throw AuthErrorFactory.createError(AuthErrorCode.PROVIDER_ERROR, 'Google authentication failed', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
...
class SMSLoginUseCase {
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('quota-exceeded')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.SMS_QUOTA_EXCEEDED, 'SMS quota exceeded. Please try again later.');
                }
                if (error.message.includes('invalid-phone-number')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.INVALID_PHONE_NUMBER, 'Invalid phone number format.');
                }
                throw AuthErrorFactory.createError(AuthErrorCode.INTERNAL_ERROR, 'Failed to send SMS verification', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
        ...
        try {
            ...
            if (error instanceof Error) {
                ...
                if (error.message.includes('invalid-verification-code')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.INVALID_VERIFICATION_CODE, 'Invalid verification code. Please check and try again.');
                }
                if (error.message.includes('expired')) {
                    throw AuthErrorFactory.createError(AuthErrorCode.TOKEN_EXPIRED, 'Verification code has expired. Please request a new one.');
                }
                throw AuthErrorFactory.createError(AuthErrorCode.INTERNAL_ERROR, 'SMS verification failed', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
    async validatePhoneNumber(phoneNumber, countryCode) {
        ...
        if (!validation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.INVALID_PHONE_NUMBER, validation.errors.join(', '));
        }
    }
    async validateVerificationCode(code) {
        const validation = this.validationService.validateSMSCode(code);
        if (!validation.isValid) {
            throw AuthErrorFactory.createError(AuthErrorCode.INVALID_VERIFICATION_CODE, validation.errors.join(', '));
        }
    }
}
...
class LogoutUseCase {
    ...
    async execute(request = {}) {
        try {
            ...
            if (error instanceof Error) {
                throw AuthErrorFactory.createError(AuthErrorCode.INTERNAL_ERROR, 'Logout failed', { originalMessage: error.message }, error);
            }
            throw error;
        }
    }
...
const AuthGuard = ({ children, requireAuth = true, requireEmailVerification = false, requiredPermissions = [], fallback, loadingComponent, unauthorizedComponent, redirectTo, onUnauthorized }) => {
    ...
    if (!isInitialized || isLoading) {
        ...
        return (jsxs("div", { className: "auth-guard-loading flex items-center justify-center min-h-screen", children: [jsx("div", { className: "animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600" }), jsx("span", { className: "ml-2 text-gray-600", children: "Loading..." })] }));
    }
    ...
    if (requireAuth && !isAuthenticated) {
        ...
        if (unauthorizedComponent) {
            return jsx(Fragment, { children: unauthorizedComponent });
        }
        if (fallback) {
            return jsx(Fragment, { children: fallback });
        }
        return (jsx("div", { className: "auth-guard-unauthorized flex items-center justify-center min-h-screen", children: jsxs("div", { className: "text-center", children: [jsx("h2", { className: "text-xl font-semibold text-gray-900 mb-2", children: "Authentication Required" }), jsx("p", { className: "text-gray-600", children: "Please sign in to access this page." })] }) }));
    }
    ...
    if (requireEmailVerification && user && !user.emailVerified) {
        ...
        return (jsx("div", { className: "auth-guard-email-verification flex items-center justify-center min-h-screen", children: jsxs("div", { className: "text-center max-w-md mx-auto px-4", children: [jsx("div", { className: "mb-4", children: jsx("svg", { className: "h-12 w-12 text-yellow-500 mx-auto", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" }) }) }), jsx("h2", { className: "text-xl font-semibold text-gray-900 mb-2", children: "Email Verification Required" }), jsx("p", { className: "text-gray-600 mb-4", children: "Please verify your email address to access this page. Check your inbox for a verification link." }), jsx("button", { onClick: () => {
    ...
    if (requiredPermissions.length > 0 && user) {
        ...
        if (!hasAllPermissions) {
            ...
            return (jsx("div", { className: "auth-guard-insufficient-permissions flex items-center justify-center min-h-screen", children: jsxs("div", { className: "text-center", children: [jsx("div", { className: "mb-4", children: jsx("svg", { className: "h-12 w-12 text-red-500 mx-auto", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M12 15v2m-6 0h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" }) }) }), jsx("h2", { className: "text-xl font-semibold text-gray-900 mb-2", children: "Insufficient Permissions" }), jsx("p", { className: "text-gray-600", children: "You don't have the required permissions to access this page." })] }) }));
        }
    }
...
const ProtectedRoute = ({ children, fallback }) => (jsx(AuthGuard, { requireAuth: true, fallback: fallback, children: children }));
const PublicRoute = ({ children }) => (jsx(AuthGuard, { requireAuth: false, children: children }));
const VerifiedRoute = ({ children, fallback }) => (jsx(AuthGuard, { requireAuth: true, requireEmailVerification: true, fallback: fallback, children: children }));
...
const ForgotPasswordForm = ({ onSuccess, onError, onCancel, className = '', disabled = false, showBackToLogin = true }) => {
    ...
    if (isEmailSent) {
        return (jsx("div", { className: `forgot-password-form ${className}`, children: jsxs("div", { className: "text-center space-y-4", children: [jsx("div", { className: "w-16 h-16 mx-auto bg-green-100 rounded-full flex items-center justify-center", children: jsx("svg", { className: "w-8 h-8 text-green-600", fill: "none", stroke: "currentColor", viewBox: "0 0 24 24", children: jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 2, d: "M5 13l4 4L19 7" }) }) }), jsxs("div", { children: [jsx("h3", { className: "text-lg font-medium text-gray-900 mb-2", children: "Email Sent Successfully" }), jsxs("p", { className: "text-sm text-gray-600 mb-4", children: ["We've sent a password reset link to ", jsx("strong", { children: email }), ". Please check your inbox and follow the instructions to reset your password."] }), jsx("p", { className: "text-xs text-gray-500", children: "Didn't receive the email? Check your spam folder or try again." })] }), jsxs("div", { className: "space-y-2", children: [jsx("button", { type: "button", onClick: handleTryAgain, className: "w-full px-4 py-2 text-sm font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2", children: "Send Another Email" }), showBackToLogin && (jsx("button", { type: "button", onClick: onCancel, className: "w-full px-4 py-2 text-sm font-medium text-gray-600 bg-gray-50 border border-gray-200 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2", children: "Back to Login" }))] })] }) }));
    }
    return (jsx("div", { className: `forgot-password-form ${className}`, children: jsxs("form", { onSubmit: handleSubmit, className: "space-y-4", children: [jsxs("div", { children: [jsx("label", { htmlFor: "email", className: "block text-sm font-medium text-gray-700 mb-1", children: "Email Address" }), jsx("input", { type: "email", id: "email", name: "email", value: email, onChange: handleInputChange, disabled: isFormDisabled, className: `w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.email
                                ? 'border-red-300 focus:ring-red-500 focus:border-red-500'
                                : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-50 cursor-not-allowed' : ''}`, placeholder: "Enter your email address", autoComplete: "email", autoFocus: true }), errors.email && (jsx("p", { className: "mt-1 text-xs text-red-600", children: errors.email }))] }), errors.general && (jsx("div", { className: "p-3 bg-red-50 border border-red-200 rounded-md", children: jsx("p", { className: "text-sm text-red-600", children: errors.general }) })), jsxs("div", { className: "space-y-2", children: [jsx("button", { type: "submit", disabled: isFormDisabled, className: `w-full px-4 py-2 text-sm font-medium text-white rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${isFormDisabled
                                ? 'bg-gray-400 cursor-not-allowed'
                                : 'bg-blue-600 hover:bg-blue-700'}`, children: isSubmitting ? (jsxs("span", { className: "flex items-center justify-center", children: [jsxs("svg", { className: "animate-spin -ml-1 mr-2 h-4 w-4 text-white", fill: "none", viewBox: "0 0 24 24", children: [jsx("circle", { className: "opacity-25", cx: "12", cy: "12", r: "10", stroke: "currentColor", strokeWidth: "4" }), jsx("path", { className: "opacity-75", fill: "currentColor", d: "M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" })] }), "Sending..."] })) : ('Send Reset Email') }), showBackToLogin && (jsx("button", { type: "button", onClick: onCancel, disabled: isFormDisabled, className: "w-full px-4 py-2 text-sm font-medium text-gray-600 bg-gray-50 border border-gray-200 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50", children: "Back to Login" }))] })] }) }));
};
...
const LoginForm = ({ onSuccess, onError, showRememberMe = true, showGoogleLogin = true, showSMSLogin = false, className = '', disabled = false }) => {
    ...
    const isFormDisabled = isLoading || isSubmitting || disabled;
    return (jsxs("div", { className: `auth-login-form ${className}`, children: [jsxs("form", { onSubmit: handleSubmit, className: "space-y-4", children: [errors.general && (jsx("div", { className: "auth-error-message text-red-600 text-sm", children: errors.general })), jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "email", className: "block text-sm font-medium text-gray-700", children: "Email" }), jsx("input", { type: "email", id: "email", name: "email", value: formData.email, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.email ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your email" }), errors.email && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.email }))] }), jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "password", className: "block text-sm font-medium text-gray-700", children: "Password" }), jsx("input", { type: "password", id: "password", name: "password", value: formData.password, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.password ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your password" }), errors.password && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.password }))] }), showRememberMe && (jsx("div", { className: "form-group", children: jsxs("label", { className: "flex items-center", children: [jsx("input", { type: "checkbox", name: "rememberMe", checked: formData.rememberMe, onChange: handleInputChange, disabled: isFormDisabled, className: "h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded" }), jsx("span", { className: "ml-2 text-sm text-gray-700", children: "Remember me" })] }) })), jsx("button", { type: "submit", disabled: isFormDisabled, className: `w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${isFormDisabled
                            ? 'bg-gray-400 cursor-not-allowed'
                            : 'bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'}`, children: isSubmitting ? 'Signing in...' : 'Sign in' })] }), showGoogleLogin && (jsxs("div", { className: "mt-4", children: [jsxs("div", { className: "relative", children: [jsx("div", { className: "absolute inset-0 flex items-center", children: jsx("div", { className: "w-full border-t border-gray-300" }) }), jsx("div", { className: "relative flex justify-center text-sm", children: jsx("span", { className: "px-2 bg-white text-gray-500", children: "Or continue with" }) })] }), jsxs("button", { type: "button", onClick: handleGoogleLogin, disabled: isFormDisabled, className: `mt-3 w-full flex justify-center items-center py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium ${isFormDisabled
                            ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                            : 'bg-white text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500'}`, children: [jsxs("svg", { className: "w-5 h-5 mr-2", viewBox: "0 0 24 24", children: [jsx("path", { fill: "currentColor", d: "M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" }), jsx("path", { fill: "currentColor", d: "M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" }), jsx("path", { fill: "currentColor", d: "M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" }), jsx("path", { fill: "currentColor", d: "M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" })] }), "Continue with Google"] })] }))] }));
};
...
const RegisterForm = ({ onSuccess, onError, showDisplayName = true, showPhoneNumber = false, requireTermsAcceptance = true, className = '', disabled = false }) => {
    ...
    const isFormDisabled = isLoading || isSubmitting || disabled;
    return (jsx("div", { className: `auth-register-form ${className}`, children: jsxs("form", { onSubmit: handleSubmit, className: "space-y-4", children: [errors.general && (jsx("div", { className: "auth-error-message text-red-600 text-sm", children: errors.general })), jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "email", className: "block text-sm font-medium text-gray-700", children: "Email *" }), jsx("input", { type: "email", id: "email", name: "email", value: formData.email, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.email ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your email" }), errors.email && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.email }))] }), showDisplayName && (jsxs("div", { className: "form-group", children: [jsxs("label", { htmlFor: "displayName", className: "block text-sm font-medium text-gray-700", children: ["Display Name ", showDisplayName ? '*' : ''] }), jsx("input", { type: "text", id: "displayName", name: "displayName", value: formData.displayName, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.displayName ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your display name" }), errors.displayName && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.displayName }))] })), showPhoneNumber && (jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "phoneNumber", className: "block text-sm font-medium text-gray-700", children: "Phone Number" }), jsx("input", { type: "tel", id: "phoneNumber", name: "phoneNumber", value: formData.phoneNumber, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.phoneNumber ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your phone number" }), errors.phoneNumber && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.phoneNumber }))] })), jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "password", className: "block text-sm font-medium text-gray-700", children: "Password *" }), jsx("input", { type: "password", id: "password", name: "password", value: formData.password, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.password ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Enter your password" }), errors.password && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.password })), jsx("p", { className: "mt-1 text-xs text-gray-500", children: "Password must be at least 8 characters with uppercase, lowercase, and number" })] }), jsxs("div", { className: "form-group", children: [jsx("label", { htmlFor: "confirmPassword", className: "block text-sm font-medium text-gray-700", children: "Confirm Password *" }), jsx("input", { type: "password", id: "confirmPassword", name: "confirmPassword", value: formData.confirmPassword, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${errors.confirmPassword ? 'border-red-500' : 'border-gray-300'} ${isFormDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`, placeholder: "Confirm your password" }), errors.confirmPassword && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.confirmPassword }))] }), requireTermsAcceptance && (jsxs("div", { className: "form-group", children: [jsxs("label", { className: "flex items-start", children: [jsx("input", { type: "checkbox", name: "acceptTerms", checked: formData.acceptTerms, onChange: handleInputChange, disabled: isFormDisabled, className: `mt-1 h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded ${errors.acceptTerms ? 'border-red-500' : ''}` }), jsxs("span", { className: "ml-2 text-sm text-gray-700", children: ["I accept the", ' ', jsx("a", { href: "/terms", className: "text-blue-600 hover:text-blue-500", target: "_blank", rel: "noopener noreferrer", children: "Terms and Conditions" }), ' ', "and", ' ', jsx("a", { href: "/privacy", className: "text-blue-600 hover:text-blue-500", target: "_blank", rel: "noopener noreferrer", children: "Privacy Policy" })] })] }), errors.acceptTerms && (jsx("p", { className: "mt-1 text-sm text-red-600", children: errors.acceptTerms }))] })), jsx("button", { type: "submit", disabled: isFormDisabled, className: `w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white ${isFormDisabled
                        ? 'bg-gray-400 cursor-not-allowed'
...
export { AuthConfigValidator, AuthErrorCode, AuthErrorFactory, AuthGuard, AuthProvider, AuthStateStatus, DIContainer, FirebaseConfigService, ForgotPasswordForm, GoogleLoginUseCase, LocalStorageService, LoginForm, LoginUseCase, LogoutUseCase, ProtectedRoute, PublicRoute, RegisterForm, RegisterUseCase, SMSLoginUseCase, ValidationServiceImpl, VerifiedRoute, useAuth, useAuthActions, useAuthContext, useAuthState };
